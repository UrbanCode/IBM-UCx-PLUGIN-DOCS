...
| `<sql>` | A child of the `<change>` element or `<rollback>` element. The element contains the SQL statements that are required to apply the change. Within the `<SQL>` tags are the SQL statements that are required to apply the change to the database. Each SQL statement must be separated by the separator that is defined in the separator attribute. The SQL statements can be defined in a separate file, which is identified by using the file attribute. | separator | Optional. The default value is two semicolons `;;`. This element defines how to separate each SQL statement. For example, if the separator is `;;`, then each SQL statement must end with `;;`. |
